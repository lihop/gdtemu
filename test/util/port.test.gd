...
func test_returns_null_if_no_port_available():
	var used_port := Port.get_unused_port()
	var server := TCP_Server.new()
	server.listen(used_port)
	assert_null(Port.get_unused_port(used_port, used_port))


func test_returns_null_if_port_max_out_of_range():
	assert_null(Port.get_unused_port(90000000, 99999999))


func test_returns_null_if_port_min_out_of_range():
	assert_null(Port.get_unused_port(-1, 5050))


func test_returns_null_if_port_min_greater_than_port_max():
	assert_null(Port.get_unused_port(5050, 5049))
